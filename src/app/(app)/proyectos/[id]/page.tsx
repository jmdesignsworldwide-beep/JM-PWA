...
import { getVentureById, getVentureFileUrl } from "@/lib/data/ventures";
...
export default async function VenturePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [logoUrl, todos] = await Promise.all([
    getVentureFileUrl(venture.logo_path),
    getVentureTodos(id),
  ]);

  return <VentureDetail venture={venture} logoUrl={logoUrl} todos={todos} />;
}
